...
12. [传统需求覆盖度分析](#第十二章--传统需求覆盖度分析)
13. [实施指南与最佳实践](#第十三章--实施指南与最佳实践)
14. [附录：术语对照表](#附录--术语对照表)
...
在对象、行为和规则等基础模型之上，独立定义事件、事件协同场景、权限主体、业务流程和查询报表，形成八模型体系。v5.0 在此基础上**新增第九模型 MU（UI 模型）**，构成九模型体系；本版再**配套 MM 对象-数据表映射模型**，表达对象与数据库表/字段的持久化对应关系：
...
八个模型之间存在依赖，建议按以下顺序建模。M5分两次完善，因此阶段数多于模型数。v5.0 增加第 10 阶段 **MU UI 模型**，放在最后以业务模型 ID 稳定后再建模 UI，可避免频繁回改；本版补充第 11 阶段 **MM 映射模型**，在 M1 与数据库 DDL 稳定后建立对象→表、属性→列的持久化映射：
...
平台源代码、数据库和模型文件统一位于 `onto-cs` 目录。导入外部模型文件夹时，平台必须将文件复制到托管项目目录，不直接修改外部源文件。目录约定如下（v5.0 新增 `mu-ui-model.yaml`，本版配套新增 `m-mapping-model.yaml`）：
...
- 导入时先校验十类固定文件；通过后复制到 `onto-cs/model/{projectId}/yaml/`
- 导入源目录仅作为输入，不与托管项目建立文件链接，也不在原目录回写
- `manifest.json` 为可选文件；缺失时按十个固定文件名识别，存在时不得保存绝对路径
...
| 可视化 | 分别生成事件协同图和流程图；M6流程图应展示角色泳道、网关、子流程和审批结果；MU 生成屏幕导航图、调用链图及 ASCII 布局图；MM 生成对象-表对照图（对象→表→列） |
| 一致性检查 | 检查行为ownerEntity、事件生产消费、规则双向引用、M4场景链、M6流程引用以及M2/M7一对一绑定；检查 MU 调用链门禁（§10.4）与布局元素一致性（§10.6.5）；检查 MM 映射一致性（§11.5） |
| 代码生成 | 生成实体骨架、接口签名、事件DTO、权限枚举、查询DTO、Query Service及报表导出骨架；由 MU 生成屏幕骨架、事件处理器签名、导航路由与编排调用 |
| 变更影响分析 | M1变更时分析M2、M3、ME、M4、M6和M7；M2查询行为变更时检查M7；M5角色变更时检查M6人工活动；MU事件变更时分析M2行为/M3规则/M7报表；M2行为删除时分析引用它的MU事件 |
| 模型验证CI | 校验事件链、流程可达性、角色引用、网关分支、子流程调用环、M7来源字段及M2/M7一对一关系；校验 MU `uiEventRefs`/`uiValidationRefs`/`uiScreenRefs` 反向可追溯 |
...
*© 2026  Ontology-Driven Software Modeling Framework  v5.0（九模型 + MM 映射模型版）*
